...
export default function DriverSignup({ onSwitchToLogin, onBackToRoleSelector }: DriverSignupProps) {
  const { signUp, loading, error, clearError, sendOTP, verifyOTP, resendOTP } = useDriverAuth();
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.phone.length < 10) errors.push('Please enter a valid phone number');
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.content}>
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Start Delivering Today</Text>
          </View>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput style={styles.input} placeholder="driver@example.com" value={formData.email} onChangeText={(v) => updateField('email', v)} autoCapitalize="none" keyboardType="email-address" editable={!loading} />
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput style={styles.input} placeholder="DL123456" value={formData.licenseNumber} onChangeText={(v) => updateField('licenseNumber', v)} autoCapitalize="characters" editable={!loading} />
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.vehicleContainer}>
                {(['motorcycle', 'car', 'van'] as const).map((type) => (
                  <TouchableOpacity key={type} style={[styles.vehicleButton, formData.vehicleType === type && styles.vehicleButtonActive]} onPress={() => updateField('vehicleType', type)} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  eyeIcon: { fontSize: 20 },
  vehicleContainer: { flexDirection: 'row', gap: 8 },
  vehicleButton: { flex: 1, borderWidth: 1, borderColor: '#DDE1E5', borderRadius: 8, padding: 12, alignItems: 'center' },
  vehicleButtonActive: { backgroundColor: '#45B7D1', borderColor: '#45B7D1' },
  vehicleButtonText: { fontSize: 12, color: '#7F8C8D' },
  vehicleButtonTextActive: { color: '#FFF', fontWeight: 'bold' },
  signupButton: { backgroundColor: '#45B7D1', borderRadius: 8, padding: 16, alignItems: 'center', marginTop: 8 },
  signupButtonDisabled: { backgroundColor: '#BDC3C7' },
  signupButtonText: { color: '#FFF', fontSize: 16, fontWeight: 'bold' },
  footer: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 24 },
  footerText: { color: '#7F8C8D', fontSize: 14 },
  loginLink: { color: '#45B7D1', fontSize: 14, fontWeight: 'bold' },
